  ...
  _createClass(CanvasEntry, [{
    ...
    value: function drawLineToContext(ctx, peaks, absmax, halfH, offsetY, start, end) {
      ...
      for (i = canvasStart; i < canvasEnd; i++) {
        ...
        h = peak / absmaxHalf;
        //console.log('hnormal= '+h);

        if( h == 0 ){
            h = (Math.random() * (30 - 10) + 10);
            console.log('hnormal= '+h);
        }else if( h <= 0.01 ){
            // console.log('hcase h <= 0.1');
            h = h*50;
        }else if( h <= 0.1 ){
            // console.log('hcase h <= 0.1');
            h = h*20;
        }else if( h <= 1 ){
            // console.log('hcase h <= 1');
            h = h*10;
        }else if( h <= 5 ){
            // console.log('hcase h <= 5');
            h = h*2;
        }else if( h <= 6 ){
            // console.log('hcase h <= 6');
            h = h*1.5;
        }else if( h <= 10 ){
            // console.log('hcase h <= 1');
            h = h*1.2;
...
var Drawer = /*#__PURE__*/function (_util$Observer) {
  ...
  _createClass(Drawer, [{
    ...
    value: function recenterOnPosition(position, immediate) {
      ...
      var half = ~~(750 / 2);
      var maxScroll = this.wrapper.scrollWidth - 750;
    ...
    value: function getWidth() {
      return Math.round(1200 * this.params.pixelRatio);
    }
    /**
  ...
  _createClass(MultiCanvas, [{
      ...
      return this.prepareDraw(peaks, channelIndex, start, end, function (_ref) {
        ...
        for (i; i < last; i += step) {
           ...
           * always draw *something* (normally a 1px high bar) */
            ...
            if( (h <= 0 && ispos == 1) || (h >= 0 && ispos == 0) ){
                h = 0;
            }else if( (h <= 0.01 && ispos == 1)  || (h >= 0.01 && ispos == 0) ){
                // console.log('hcase h <= 0.1');
                h = h*50;
            }else if((h <= 0.1 && ispos == 1)  || (h >= 0.1 && ispos == 0) ){
                // console.log('hcase h <= 0.1');
                h = h*20;
            }else if( (h <= 1 && ispos == 1)  || (h >= 1 && ispos == 0) ){
                // console.log('hcase h <= 1');
                h = h*10;
            }else if( (h <= 5 && ispos == 1)  || (h >= 5 && ispos == 0) ){
                // console.log('hcase h <= 5');
                h = h*2;
            }else if( (h <= 6 && ispos == 1)  || (h >= 6 && ispos == 0) ){
                // console.log('hcase h <= 6');
                h = h*1.5;
            }else if( (h <= 10 && ispos == 1)  || (h >= 10 && ispos == 0) ){
                // console.log('hcase h <= 1');
                h = h*1.2;
            }else{
                h = h;
    ...
    value: function drawWave(peaks, channelIndex, start, end) {
      ...
      return this.prepareDraw(peaks, channelIndex, start, end, function (_ref) {
        ...
        var peakIndexScale = hasMinVals ? 2 : 1;
        var length = peaks.length / peakIndexScale;
        var bar = 0.5 * _this5.params.pixelRatio;
        var gap = Math.max(_this5.params.pixelRatio, 2 * _this5.params.pixelRatio);
        var step = bar;
        var scale = length / _this5.width;
        var first = start;
        var last = end;
        var i = first;
        ...
        for (i; i < last; i += step) {
          ...
          //if (h == 0 && _this5.params.barMinHeight) h = _this5.params.barMinHeight;
          if (h == 0) h = (Math.random() * (20 - 5) + 5);

          _this5.fillRect(i + _this5.halfPixel, halfH - h + offsetY, bar + _this5.halfPixel, h * 2, _this5.barRadius);
        }
      });
    }
    /**
...
var WaveSurfer = /*#__PURE__*/function (_util$Observer) {
  ...
  function WaveSurfer(params) {
    ...
    _this._onResize = util.debounce(function () {
      if (prevWidth != 750 && !_this.params.scrollParent) {
        prevWidth = 750;
